...
func (e *Engine) newGameCacheFile(meta *metainfo.MetaInfo) {
	// create .game file
	infohash := meta.HashInfoBytes().HexString()
	if w, err := os.Stat(e.cacheDir); err == nil && w.IsDir() {
		cacheFilePath := filepath.Join(e.cacheDir,
			fmt.Sprintf("%s%s.game", cacheSavedPrefix, infohash))
		...
		if _, err := os.Stat(cacheFilePath); os.IsNotExist(err) {
			...
			if err == nil {
				...
				log.Println("created game cache file", infohash)
			} else {
				log.Println("failed to create game file", err)
			}
		}
	}
}
...
func (e *Engine) removeMagnetCache(infohash string) {
	// remove both magnet and game cache if exists.
	cacheInfoPath := filepath.Join(e.cacheDir,
		fmt.Sprintf("%s%s.info", cacheSavedPrefix, infohash))
...
func (e *Engine) removeGameCache(infohash string, toTrash bool) {
	fileName := fmt.Sprintf("%s%s.game", cacheSavedPrefix, infohash)
	...
	if toTrash {
		...
		if err := os.Rename(cacheFilePath, trashFilePath); err == nil {
			log.Printf("move game file to trash [%s]", trashFilePath)
		} else {
			log.Println("fail to move to trash", err)
		}
	} else {
		if err := os.Remove(cacheFilePath); err == nil {
			log.Printf("removed game file [%s]", cacheFilePath)
		} else if !os.IsNotExist(err) { // it's fine if the cache is not exists
			log.Printf("fail to removed cache file [%s] %s", infohash, err)
		}
	}
...
func (e *Engine) GameCacheFileName(infohash string) string {
	cacheFilePath := filepath.Join(e.cacheDir,
		fmt.Sprintf("%s%s.game", cacheSavedPrefix, infohash))
...
func (e *Engine) PushWaitTask(ih string) error {
	...
	e.pushWaitTask(ih, taskGame)
	info, err := metainfo.LoadFromFile(e.GameCacheFileName(ih))
	if err != nil {
		return err
	}
	spec := game.GameSpecFromMetaInfo(info)
	_, err = e.upsertGame(ih, spec.DisplayName, true)
	return err
}
...
func (e *Engine) RestoreTask(fn string) error {
	...
	if strings.HasSuffix(fn, ".game") {
		if err := e.NewGameByFilePath(fn); err != nil {
			return err
		}
		if isCachedFile {
			log.Printf("[RestoreTask] Restored Game: %s \n", fn)
		} else {
...
func (e *Engine) NextWaitTask() error {
	...
	for {
		if elm := e.waitList.Pop(); elm != nil {
			...
			switch te.tp {
			case taskGame:
				res = fmt.Sprintf("%s%s.game", cacheSavedPrefix, te.ih)
			case taskMagnet:
