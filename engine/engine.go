...
import (
	...
	"github.com/anacrolix/game/metainfo"
	"github.com/anacrolix/game/storage"
	"github.com/boypt/simple-game/common"
...
const (
	CachedGameDir = ".cachedGames"
	TrashGameDir  = ".trashGames"
)
...
//the Engine Cloud Game engine, backed by anacrolix/game
type Engine struct {
...
func (e *Engine) Configure(c *Config) error {
	...
	e.Lock()
	defer e.Unlock()
	tc := game.NewDefaultClientConfig()
	...
	tc.HeaderObfuscationPolicy = game.HeaderObfuscationPolicy{
...

	{
		...
		for max > 0 {
			max--
			e.client, err = game.NewClient(tc)
			if err == nil {
				break
			}
	...
	e.cacheDir = path.Join(c.DownloadDirectory, CachedGameDir)
	e.trashDir = path.Join(c.DownloadDirectory, TrashGameDir)
	mkdir(e.cacheDir)
	mkdir(e.trashDir)
	e.config = *c
	return nil
...
// NewMagnet -> newGameBySpec
func (e *Engine) NewMagnet(magnetURI string) error {
	log.Println("[NewMagnet] called:", magnetURI)
	spec, err := game.GameSpecFromMagnetUri(magnetURI)
	if err != nil {
		return err
	}
	e.newMagnetCacheFile(magnetURI, spec.InfoHash.HexString())
	return e.newGameBySpec(spec, taskMagnet)
}

// NewGameByReader -> newGameBySpec
func (e *Engine) NewGameByReader(r io.Reader) error {
	info, err := metainfo.Load(r)
	if err != nil {
		return err
	}
	spec := game.GameSpecFromMetaInfo(info)
	e.newGameCacheFile(info)
	return e.newGameBySpec(spec, taskGame)
}

// NewGameByFilePath -> newGameBySpec
func (e *Engine) NewGameByFilePath(path string) error {
	// game.GameSpecFromMetaInfo may panic if the info is malformed
	defer func() error {
		if r := recover(); r != nil {
			err := fmt.Errorf("Error loading new game from file %s: %+v", path, r)
			log.Println(err)
			return err
		}
		return nil
...
func (e *Engine) isReadyAddTask() bool {
	nowGamesLen := len(e.client.Games())
	if e.config.MaxConcurrentTask > 0 && nowGamesLen >= e.config.MaxConcurrentTask {
		return false
	}
	return true
}

// NewGameBySpec -> *Game -> addGameTask
func (e *Engine) newGameBySpec(spec *game.GameSpec, taskT taskType) error {
	ih := spec.InfoHash.HexString()
	log.Println("[newGameBySpec] called", ih)
	...
	if !e.isReadyAddTask() {
		if !e.isTaskInList(ih) {
			log.Printf("[newGameBySpec] reached max task %d, add as pretask: %s %v", e.config.MaxConcurrentTask, ih, taskT)
			e.pushWaitTask(ih, taskT)
		} else {
			log.Printf("[newGameBySpec] reached max task %d, task already in queue: %s %v", e.config.MaxConcurrentTask, ih, taskT)
		}
		_, err := e.upsertGame(ih, spec.DisplayName, true) // show queueing task
		common.FancyHandleError(err)
		return ErrMaxConnTasks
	}

	t, _ := e.upsertGame(ih, spec.DisplayName, false)
	tt, _, err := e.client.AddGameSpec(spec)
	if err != nil {
		return err
	}
	...
	if len(e.Trackers) > 0 && (e.config.AlwaysAddTrackers || len(meta.AnnounceList) == 0) {
		log.Printf("[newGame] added %d public trackers\n", len(e.Trackers))
		tt.AddTrackers([][]string{e.Trackers})
	}

	go e.gameEventProcessor(tt, t, ih)
	return nil
}

func (e *Engine) gameEventProcessor(tt *game.Game, t *Game, ih string) {
	...
	case <-t.dropWait:
		...
		return
	case <-tt.GotInfo():
		// Already got full game info
		// If the origin is from a magnet link, remove it, cache the game data
		e.removeMagnetCache(ih)
		m := tt.Metainfo()
		e.newGameCacheFile(&m)
		t.updateOnGotInfo(tt)
		e.TsChanged <- struct{}{}
	}

	if e.config.AutoStart {
		go e.StartGame(ih) // nolint: errcheck
	}

	timeTk := time.NewTicker(3 * time.Second)
	defer timeTk.Stop()

	// main loop updating the game status to our struct
	for {
		select {
		case <-timeTk.C:
			if !t.IsAllFilesDone {
...
//GetGames just get the local infohash->Game map
func (e *Engine) GetGames() *map[string]*Game {
	return &e.ts
}

// TaskRoutine
func (e *Engine) taskRoutine(t *Game) {
...
func (e *Engine) stopRemoveTask(ih string) {
	common.FancyHandleError(e.StopGame(ih))
	e.RemoveCache(ih)
	common.FancyHandleError(e.DeleteGame(ih))
}

func (e *Engine) ManualStartGame(infohash string) error {
	if err := e.StartGame(infohash); err == nil {
		t, _ := e.getGame(infohash)
		t.Lock()
		defer t.Unlock()
...
func (e *Engine) StartGame(infohash string) error {
	log.Println("StartGame", infohash)
	e.Lock()
	defer e.Unlock()

	t, err := e.getGame(infohash)
	if err != nil {
		return err
	}
	t.Lock()
...
func (e *Engine) StopGame(infohash string) error {
	log.Println("StopGame", infohash)
	e.Lock()
	defer e.Unlock()
	t, err := e.getGame(infohash)
	if err != nil {
...
func (e *Engine) DeleteGame(infohash string) error {
	log.Println("DeleteGame", infohash)
	e.Lock()
	defer e.Unlock()
...
func (e *Engine) StartFile(infohash, filepath string) error {
	...
	f.Started = true
	f.f.SetPriority(game.PiecePriorityNormal)
	return nil
}
...
func (e *Engine) StopFile(infohash, filepath string) error {
	...
	f.Started = false
	f.f.SetPriority(game.PiecePriorityNone)
...
func (e *Engine) RemoveCache(infohash string) {
	...
	e.removeGameCache(infohash, true)
}
