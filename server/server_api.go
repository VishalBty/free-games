...
import (
	...
	"github.com/boypt/simple-game/engine"
)
...
func (s *Server) apiGET(w http.ResponseWriter, r *http.Request) error {
	...
	case "games":
		common.HandleError(json.NewEncoder(w).Encode(s.engine.GetGames()))
	case "files":
...
func (s *Server) apiPOST(r *http.Request) error {
	...
	//convert url into game bytes
	if action == "url" {
		url := string(data)
		remote, err := http.Get(url)
		if err != nil {
			return fmt.Errorf("ERROR: Invalid remote game URL: %s %w", url, err)
		}
		defer remote.Body.Close()
		if remote.ContentLength > 512*1024 {
			//enforce max body size (512k)
			return fmt.Errorf("ERROR: Remote game too large")
		}
		data, err = ioutil.ReadAll(remote.Body)
		if err != nil {
			return fmt.Errorf("ERROR: Failed to download remote game: %w", err)
		}
		action = "gamefile"
	}

	//convert game bytes into magnet
	if action == "gamefile" {
		if err := s.engine.NewGameByReader(bytes.NewBuffer(data)); err != nil {
			if !errors.Is(err, engine.ErrMaxConnTasks) {
				return err
			}
		}
	...
	case "game":
		...
		switch state {
		case "start":
			if err := s.engine.ManualStartGame(infohash); err != nil {
				return err
			}
		case "stop":
			if err := s.engine.StopGame(infohash); err != nil {
				return err
			}
		case "delete":
			if err := s.engine.DeleteGame(infohash); err != nil {
				return err
			}
...
func (s *Server) apiConfigure(data []byte) error {
	...
	if !reflect.DeepEqual(s.engineConfig, c) {
		...
		if status&engine.NeedRestartWatch > 0 {
			common.FancyHandleError(s.engine.StartGameWatcher())
		...
		if status&engine.NeedEngineReConfig > 0 {
			...
			log.Printf("[api] game engine reconfigred")
		} else {
