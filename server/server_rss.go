...
var (
	...
	gameExp  = regexp.MustCompile(`\.game`)
)
...
func (ritem *rssJSONItem) findFromFeedItem(i *gofeed.Item) (found bool) {
	...
	for _, ex := range []string{"game", "nyaa"} {
	...
	for _, e := range i.Enclosures {
		if strings.HasPrefix(e.URL, "magnet:") {
			ritem.Magnet = e.URL
		} else if gameExp.Match([]byte(e.URL)) {
			ritem.Game = e.URL
		}
	}

	// maybe the Link is a game file
	if gameExp.MatchString(i.Link) {
		ritem.Game = i.Link
	}

	// not found magnet/game, try to find them in the description
	if ritem.Magnet == "" && ritem.InfoHash == "" && ritem.Game == "" {
	...
	return (ritem.Magnet != "" || ritem.InfoHash != "" || ritem.Game != "")
}

func (r *rssJSONItem) readExtention(i *gofeed.Item, ext string) (found bool) {

	// There are no starndards for rss feeds contains games or magnets
