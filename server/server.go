...
import (
	...
	"github.com/boypt/simple-game/server/httpmiddleware"
	...
	ctstatic "github.com/boypt/simple-game/static"
...
type Server struct {
	...
	ConfigPath     string `opts:"help=Configuration file path (default ./cloud-game.yaml),short=c,env=CONFIGPATH"`
	...
	DebugGame   bool   `opts:"help=Debug game engine,env=DEBUGGAME"`
...
func (s *Server) Run(tpl *TPLInfo) error {
	...
	c.EngineDebug = s.DebugGame

	// write cloud-game.yaml at the same dir with -c conf and exit
	if s.ConvYAML {
	...
	s.state.Games = s.engine.GetGames()
