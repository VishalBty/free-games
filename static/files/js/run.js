...
app.run(function ($rootScope, $window, $location, $log, search, api, apiget, storage, reqinfo, reqerr) {
  ...
  if ("registerProtocolHandler" in $window.navigator) {
    ...
    $window.navigator.registerProtocolHandler("magnet", handlurl, "SimpleGame");
  ...
  $scope.uploadGame = function (event) {
    ...
    var files = filter.call(fileContainer.files, function (file) {
      return file.name.endsWith(".game");
    });
    if (files.length === 0) {
      return $rootScope.alertErr("No game files to upload");
    }
    files.forEach(function (file) {
      ...
      reader.onload = function () {
        ...
        api.gamefile(data).then(reqinfo, reqerr);
      };
    });
  };
  ...
  document.addEventListener("keydown", function (e) {
    ...
    for (var i = 0; i < medias.length; i++) {
      ...
      var inView = (rect.top >= 0 && rect.top <= height) || (rect.bottom >= 0 && rect.bottom <= height);
      if (!inView) {
        continue;
      }
      if (media.paused) {
