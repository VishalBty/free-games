/* globals app,window */
app.controller("OmniController", function ($scope, $rootScope, storage, api, apiget, search, reqinfo, rss) {
  ...
  var parseMagnet = function (params) {
    ...
    //in place map
    for (var i = 0; i < m.tr.length; i++) $scope.magnet.trackers[i] = { v: m.tr[i] };

    while ($scope.magnet.trackers.length > m.tr.length) $scope.magnet.trackers.pop();
  ...
  $scope.parse = function () {
    ...
    if (/^https?:\/\//.test($scope.inputs.omni)) parseGame();
  ...
  $scope.submitGame = function () {
    ...
    $rootScope.set_game_expanded(true);
  };
  ...
  $scope.submitSearch = function () {
    ...
    search.all($scope.inputs.provider, $scope.inputs.omni, page).then(function (xhr) {
      ...
      for (var i = 0; i < results.length; i++) {
        ...
        if (r.game && /^\//.test(r.game)) {
          r.game = origin + r.game;
        }
  ...
  $scope.submitGameItem = function (result) {
    if (result.game) {
      api.url(result.game).then(reqinfo);
    }
  };

  $scope.submitSearchItem = function (result) {
    //if search item has magnet/game, download now!
    if (result.magnet) {
    ...
    search.one($scope.inputs.provider, result.path).then(
      function (resp) {
        ...
        if (data.game) return api.url(data.game);
        var magnet;
        if (data.magnet) {
          magnet = data.magnet;
  ...
  // };
  // var files = [].slice.call(event.dataTransfer.files);
  // if (files.length === 0 || !files[0].name.endsWith(".game")) {
  //   alert("file must be a .game file");
  //   return;
  // }
